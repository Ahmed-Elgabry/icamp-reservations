                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_category_table">
                        ...
                        @foreach ($order->addons as $orderAddon)
                            <tr data-id="{{ $orderAddon->id }}">
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-light btn-active-light-primary" data-toggle="modal" data-target="#editAddonModal-{{ $orderAddon->pivot->id }}">
                                        {{ __('dashboard.edit') }} <i class="fa fa-edit"></i>
                                    </button>
                                    <form action="{{ route('orders.removeAddon', $orderAddon->pivot->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger btn-active-light-primary">
                                            {{ __('dashboard.delete') }} <i class="fa fa-trash"></i>
                                        </button>
                                    </form>
                                </td>
                            </tr>
