      ...
      <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        <div class="card-title">
          <div class="ms-3 d-flex flex-wrap gap-2">
            <a href="{{ route('pages.tasks.index', ['status' => 'all']) }}" class="btn btn-sm {{ $status === 'all' ? 'btn-primary' : 'btn-light' }}">@lang('dashboard.all')</a>
            <a href="{{ route('pages.tasks.index', ['status' => 'in_progress']) }}" class="btn btn-sm {{ $status === 'in_progress' ? 'btn-primary' : 'btn-light' }}">@lang('dashboard.in_progress')</a>
            <a href="{{ route('pages.tasks.index', ['status' => 'pending']) }}" class="btn btn-sm {{ $status === 'pending' ? 'btn-primary' : 'btn-light' }}">@lang('dashboard.pending')</a>
            <a href="{{ route('pages.tasks.index', ['status' => 'failed']) }}" class="btn btn-sm {{ $status === 'failed' ? 'btn-primary' : 'btn-light' }}">@lang('dashboard.failed')</a>
          </div>
        </div>
        <div class="card-toolbar">
          ...
          $activeTab = 'tasks';
          @endphp
          <form method="get" action="{{ route('pages.tasks.index') }}" class="d-flex align-items-center gap-2">
            ...
            <a href="{{ route('pages.tasks.index') }}" class="btn btn-sm btn-light">@lang('dashboard.clear')</a>
            @endif
          </form>
          <ul class="nav nav-tabs nav-line-tabs ms-4">
            @can('bookings.reservations.board')
            <li class="nav-item">
              <a class="nav-link {{ ($activeTab ?? 'today') === 'today' ? 'active' : '' }}" href="{{ route('pages.reservations.board', ['from' => $selectedFrom ?? null, 'to' => $selectedTo ?? null]) }}">@lang('dashboard.today_reservations')</a>
            </li>
            @endcan
            @can('bookings.reservations.board.upcoming')
            <li class="nav-item">
              <a class="nav-link {{ ($activeTab ?? 'today') === 'upcoming' ? 'active' : '' }}" href="{{ route('pages.reservations.board.upcoming', ['from' => $selectedFrom ?? null, 'to' => $selectedTo ?? null]) }}">@lang('dashboard.upcoming_reservations')</a>
            </li>
            @endcan
            @can('bookings.tasks.index')
            <li class="nav-item">
              <a class="nav-link {{ ($activeTab ?? 'today') === 'tasks' ? 'active' : '' }}" href="{{ route('pages.tasks.index') }}">@lang('dashboard.tasks')</a>
            </li>
            @endcan
          </ul>
        </div>
