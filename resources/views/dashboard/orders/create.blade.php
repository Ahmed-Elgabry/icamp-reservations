    ...
    (function() {
        ...
        $('#btnDoRetrieve').on('click', function() {
            ...
            if (!id) {
                return window.Swal ?
                    Swal.fire({
                        icon: 'warning',
                        title: @json(__('dashboard.select_required'))
                    }) :
                ...
                .fail((xhr) => {
                    window.Swal ?
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: (xhr.responseJSON && xhr.responseJSON.message) || 'Failed to fetch data.'
                        }) :
        ...
        function fillFormFromPayload(payload) {
            ...
            if (window.Swal) Swal.fire({
                icon: 'success',
                title: @json(__('dashboard.loaded_ok'))
            });
        ...
        (function linkPrefill() {
            ...
            if (window.Swal && rfId) {
                Swal.fire({
                    ...
                    text: rfId ?
                        (isRTL ? `تم سحب البيانات من الاستمارة رقم #${rfId}` : `Loaded data from form #${rfId}`) : (isRTL ? 'تم سحب البيانات من الاستمارة' : 'Loaded data from form'),
                    icon: 'success',
        ...
        tinymce.init({
            ...editorConfig,
            selector: '#showPriceEditor'
        });
        tinymce.init({
            ...editorConfig,
            selector: '#orderDataEditor'
        });
        tinymce.init({
            ...editorConfig,
            selector: '#invoiceEditor'
        });
        tinymce.init({
            ...editorConfig,
            selector: '#receiptEditor'
        });
