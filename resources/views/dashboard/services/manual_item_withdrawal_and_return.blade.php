    ...
    <script>
        ...
        (function initSelect2() {
            ...
            s.onload = function() {
                ...
                setTimeout(function() {
                    $('.select-item').trigger('resize.select2');
                }, 100);
            };
        ...
        $(document).on('click', '.btn-delete-adjustment', function() {
            ...
            function showDeleteSwal() {
                Swal.fire({
                    title: '{{ __('
                    dashboard.are_you_sure ') }}',
                    text: '{{ __('
                    dashboard.you_wont_be_able_to_revert_this ') }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#d33',
                    cancelButtonColor: '#3085d6',
                    confirmButtonText: '{{ __('
                    dashboard.yes_delete_it ') }}',
                    cancelButtonText: '{{ __('
                    dashboard.cancel ') }}'
                }).then((result) => {
                        ...
                        $.ajax({
                            ...
                            success: function() {
                                Swal.fire({
                                    ...
                                    dashboard.deleted ') }}',
                                    text: '{{ __('
                                    dashboard.deleted_successfully ') }}',
                                    icon: 'success',
                                    timer: 1200,
        ...
        $(document).ready(function() {
            ...
            $('select[name="type"]').on('change', function() {
                ...
                if (reasonOptions[selectedType]) {
                    ...
                    $reasonDropdown.html('<option value="">{{ __('
                        dashboard.manual_item_withdrawal_and_return.select_reason ') }}</option>');
                }
            });
            ...
            if ((!qty || qty === "") && (!perc || perc === "")) {
                if (window.Swal) {
                    Swal.fire({
                        ...
                        title: 'Error',
                        text: '{{ __('
                        dashboard.enter_quantity_or_percentage ') }}'
                    });
                } else {
                    alert('{{ __('
                        dashboard.enter_quantity_or_percentage ') }}');
