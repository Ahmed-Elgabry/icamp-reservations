;(function(window, $) {
    ...
    window.PhoneUtils = {
        // Object to store country codes for dial code lookup
        countryCodes: {},
        // Maximum length of a dial code
        dialCodeMaxLen: 0,

        // Initialize country codes from allCountries array
        init: function(countries) {
            this.countryCodes = {};
            countries.forEach(function(country) {
                var dialCode = country[2];
                this.countryCodes[dialCode] = true;
                this.dialCodeMaxLen = Math.max(this.dialCodeMaxLen, dialCode.length);
            }, this);
        },

        // Get the dial code from a phone number
        ...
        // Get the number without the dial code
        getNumberWithoutDialCode: function(number) {
            var dialCode = this.getDialCode(number);
            if (dialCode) {
                return number.substring(dialCode.length).trim();
            }
            return number;
        },

        // Get both dial code and number separately
        splitNumber: function(number) {
            var dialCode = this.getDialCode(number);
            var numberWithoutDial = number;

            if (dialCode) {
                numberWithoutDial = number.substring(dialCode.length).trim();
            }

            return {
                dialCode: dialCode,
                number: numberWithoutDial
            };
        }
    };

    // Initialize PhoneUtils with country data
    $(document).ready(function() {
        if (typeof allCountries !== 'undefined') {
            window.PhoneUtils.init(allCountries);
            setTimeout(() => {
                processExistingNumbers();
            }, 500);
        }
    });
    function processExistingNumbers() {
        document.querySelectorAll(`input[type="tel"]`).forEach(input => {
          if (input && input.value) {
              const fullNumber = input.value;
              const dialCode = window.PhoneUtils.getDialCode(fullNumber);
              const numberWithoutDial = window.PhoneUtils.getNumberWithoutDialCode(fullNumber);
              
              // Update the input value with just the number (without dial code)
              input.value = numberWithoutDial;
              
              // Find the select2 dropdown for this input
              const $select = $(input).siblings('select.country-select');
              
              if ($select.length) {
                  // If we have a dial code, find and select the matching country
                  if (dialCode) {
                      // Remove the + from the dial code for matching
                      $select.val(dialCode.replace(/^\+/, '')).trigger('change');
                      // // Find the option with this dial code
                      // $select.find('option').each(function() {
                      //     if ($(this).data('dial-code') === dialCodeNum) {
              ...
              // Trigger change to update any listeners
    ...
    // Initialize PhoneUtils with country data
    // allCountries data (name, iso2, dialCode, ...)
    ...
    $(function() {
        // Initialize a Select2 country selector next to any phone input
        $('input[type="tel"]').each(function(idx, el) {
            var $input = $(el).attr('data-phone-input', 'true');
            var $wrap = $('<div class="d-flex align-items-center country-select-wrap"></div>');
            $input.before($wrap);
            // put the input first, then the small select beside it
            $wrap.append($input);

            // create select element
            // small inline select; dropdown will be large using a custom dropdownCssClass
            var $select = $('<select class="country-select form-control" style="width:90px;margin-left:10px;"></select>');
            $wrap.append($select);

            // populate select2
            $select.select2({
                data: buildCountryOptions(),
                width: 'resolve',
            ...
            // set default country if data-dial attribute exists or use UAE
            var defaultDial = $input.data('dial') || '971';
            $select.val(defaultDial).trigger('change');
            ...
            $input.closest('form').on('submit', function(e) {
                ...
                // Process each phone input in the form
                $phoneInputs.each(function() {
                    var $currentInput = $(this);
                    var $select = $currentInput.siblings('select.country-select');
                    
                    // Get the selected country data
                    var selectedData = $select.select2('data');
                    if (selectedData && selectedData[0]) {
                        var dialCode = selectedData[0].id;
                        var currentVal = $currentInput.val() || '';
                        currentVal = currentVal.trim();
                        
                        // Only process if we have a value and it doesn't already start with +
                        if (currentVal && !currentVal.startsWith('+')) {
                            // Remove any leading zeros from the number
                            var cleanNumber = currentVal.replace(/^0+/, '');
                            // Combine dial code and number
                            $currentInput.val('+' + dialCode + cleanNumber);
                            
                            try { 
                                console.debug('Merged phone:', $currentInput.attr('name'), $currentInput.val()); 
                            } catch(e) {}
                        }
                    }
                });
